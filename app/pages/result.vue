...
<template>
  <div class="relative flex min-h-dvh flex-col justify-center bg-[#070a12] px-4 py-8 text-slate-100 selection:bg-sky-500/20 selection:text-sky-200 sm:py-12">
    <!-- Ambient glow -->
    <div class="pointer-events-none fixed inset-0 bg-ambient-glow" />

    <div class="relative z-10 mx-auto w-full max-w-3xl space-y-5">
      <!-- Session Header Card -->
      <div class="raycast-card rounded-2xl p-5 shadow-xl sm:p-7">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-5">
          ...
          <div class="text-left sm:text-right border-t sm:border-t-0 pt-3 sm:pt-0 border-white/[0.08]">
            <span class="text-xs text-slate-400 font-semibold block uppercase">Skor akhir</span>
        ...
        <div class="grid grid-cols-3 gap-3 pt-5">
          <div class="rounded-xl border border-white/10 bg-slate-900/60 p-3.5 text-center">
            <span class="text-[11px] font-bold text-slate-400 uppercase block">Akurasi</span>
          ...
          <div class="rounded-xl border border-white/10 bg-slate-900/60 p-3.5 text-center">
            <span class="text-[11px] font-bold text-slate-400 uppercase block">Benar</span>
          ...
          <div class="rounded-xl border border-white/10 bg-slate-900/60 p-3.5 text-center">
            <span class="text-[11px] font-bold text-slate-400 uppercase block">Streak puncak</span>
      ...
      <div class="raycast-card rounded-2xl overflow-hidden shadow-xl">
        <div class="px-5 py-3.5 border-b border-white/[0.08] flex items-center justify-between">
            <h2 class="text-xs font-bold text-white uppercase">
            Ronde tadi
          </h2>
          <span class="text-xs font-mono text-slate-400">{{ game.history.length }} ronde</span>
        </div>
          ...
          <table class="w-full text-left text-xs">
            <thead class="bg-slate-900/90 text-slate-400 sticky top-0 border-b border-white/[0.08]">
              <tr>
                ...
                <th class="px-4 py-2.5 font-semibold text-right">Poin</th>
              </tr>
            </thead>
      ...
      <div class="flex flex-col sm:flex-row items-center gap-3 pt-1">
        ...
        <button
          ...
          @click="shareResults"
        >
          {{ copiedToast ? 'Udah dicopy' : 'Copy Skor' }}
        </button>
